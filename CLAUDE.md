...
A design system is persisted at **`design-system/brandio-leather/MASTER.md`**,
generated by the **UI/UX Pro Max** skill (`.claude/skills/ui-ux-pro-max/`,
installed via `npm i -g uipro-cli` → `uipro init --ai claude`). `.claude/` is
git-ignored, so the skill is NOT in the repo — re-run `uipro init` if it's
missing. The `design-system/` folder IS committed.

Regenerate / query the engine (needs `python3`):
```bash
python3 .claude/skills/ui-ux-pro-max/scripts/search.py "luxury leather B2B" --design-system -p "Brandio Leather"
```

**Typography (applied site-wide):** headings use **Cormorant** via the
`font-display` class; body uses **Montserrat** (set as Tailwind `font-sans`).
Both are loaded in `index.html` + configured in the inline `tailwind.config`.
Use `font-display` on every new large heading.

**Brand palette stays amber/leather** — we deliberately did NOT adopt the
generator's navy palette. Keep `amber-*` / `yellow-*`. Only the typography,
effects, and checklist from the design system were adopted.

**Every new section/component MUST satisfy this checklist** (from MASTER.md):
- Responsive at **375 / 768 / 1024 / 1440px**; **no horizontal scroll on mobile**
  (wrap parallax/absolute decorations in an `overflow-hidden` parent).
- Mobile-first sizing: `text-3xl md:text-5xl`, `py-12 md:py-20`,
  `grid-cols-2 md:grid-cols-4`, full-width controls that shrink on `md:`.
- `prefers-reduced-motion` is respected globally (CSS guard in `index.html`).
- Smooth 150–300ms hover transitions; visible focus states; 4.5:1 contrast.
- Icons come from **lucide-react**, never emojis-as-icons (flags as content OK).
